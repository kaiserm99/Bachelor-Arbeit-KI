...
struct VertexConstraint {
  VertexConstraint(int time, int y, int x, int setFrom=-1) : time(time), y(y), x(x), setFrom(setFrom) {}
  int time;
  int y;
  int x;
  int setFrom;
  ...
  friend std::ostream& operator<<(std::ostream& os, const VertexConstraint& c) {
    if (c.setFrom == -1) return os << "VC(t=" << c.time << ",y=" << c.y << ",x=" << c.x << ")";

    else return os << "VC(t=" << c.time << ",y=" << c.y << ",x=" << c.x << ",a=" << c.setFrom << ")"; 
  }
};
...
class Environment {
 public:
  ...
  void addConstraints(int errorTime, size_t handle1, size_t handle2, State& criticalState, auto& solution, auto& resultConstraints, auto& resultDoubleConstraints, int type, int edge = 0) {
    ...
    else if (type == 0) handleNotApplicableJunction(errorTime, criticalState, handle1, handle2, edge, accConstraints1, accConstraints2, solution);
  ...
  void handleConflicts(const State& errorState1, const State& errorState2, const int errorTime, const size_t handle1, const size_t handle2, const auto& solution, auto& resultConstraints, auto& resultDoubleConstraints, int edge) {
    ...
    // Agent 1 cant get out of the situation 
      ...
      addConstraints(errorTime, handle2, handle1, criticalState2, solution, resultConstraints, resultDoubleConstraints, m_edges[Location(criticalState2.y, criticalState2.x, criticalState2.dir)].size() >= 2, edge);
    ...
    // Agent 2 cant get out of the situation 
      ...
      addConstraints(errorTime, handle1, handle2, criticalState1, solution, resultConstraints, resultDoubleConstraints, m_edges[Location(criticalState1.y, criticalState1.x, criticalState1.dir)].size() >= 2, edge);
  ...
  bool getFirstConflict(const auto& solution, auto& resultConstraints, auto& resultDoubleConstraints) {
        ...
        for (size_t handle2 = handle1 + 1; handle2 < solution.size(); ++handle2) {
          ...
          if (stateInizial1.equalExceptTime(stateInizial2)) {
            ...
            if (!checkHeadError(stateInizial1, stateInizial2, handle1, handle2, solution, resultConstraints)) {
              ...
              handleConflicts(stateInizial1, stateInizial2, t, handle1, handle2, solution, resultConstraints, resultDoubleConstraints, 0);
      ...
      for (size_t handle1 = 0; handle1 < solution.size(); ++handle1) {
        ...
        for (size_t handle2 = handle1 + 1; handle2 < solution.size(); ++handle2) {
          ...
          if (state1a.equalExceptTime(state2b) && state1b.equalExceptTime(state2a)) {
            ...
            handleConflicts(state1b, state2b, t+1, handle1, handle2, solution, resultConstraints, resultDoubleConstraints, 1);
  ...
  void setConstrains (const State& constrainedState, const int startingTime, const int endingTime, const size_t handle, Constraints& cons, const size_t handleOther=-1) {
    ...
    for (int timeVertex = startingTime; timeVertex <= endingTime; timeVertex++) {
      ...
      cons.vertexConstraints.emplace(VertexConstraint(timeVertex, constrainedState.y, constrainedState.x, handleOther));
  ...
  void handleApplicableJunction(const int errorTime, const State& criticalState, const size_t handle1, const size_t handle2, const int edge, auto& cons, auto& consOther, const auto& solution) {
    ...
    std::cout << std::endl << "Handle applicable:" << std::endl;
    ...
    if (errorTime == criticalState.time) {
      ...
      // From: Where Agent 2 normally got into the critical cell
      // To:   The time of the error
      setConstrains(criticalState, resPrevHandle2.second+1, errorTime, handle2, consOther, handle1);

      // If the criticalState is the goal of Agent 1, then constrain the other Agent so he will not interfere with the cell
    ...
    endingTime += stepTime - stepCriticalState - 1;


    // +2 so the Agent can get into the cell at +1 and needs to leave at +2
    const int startingTime = getPrevPosition(criticalState, handle1, solution).second+2;

    State nextSolutionState = getNextPosition(criticalState, handle1, solution).first;

    // Block the cell after the criticalState, so Agent 1 won't take this path
    setConstrains(nextSolutionState, startingTime, endingTime, handle1, cons);
  ...
  void handleNotApplicableJunction(const int errorTime, const State& criticalState, const size_t handle1, const size_t handle2, const int edge, auto& cons, auto& consOther, const auto& solution) {


    // There was already took care of from the other function, so do not expand further
    if (errorTime == criticalState.time) return;

    std::cout << std::endl << "Handle not applicable:" << std::endl;
    ...
    if (endingTime == -1) {
      ...
      // The other Agent found no other way but now he have to 
        ...
        if ((unsigned) stepCriticalState+1 == sizeUniqueSolutionHandle2) return;
        ...
        std::cout << "Heuristic Constraints (not applicable):" << std:: endl;
        ...
        if (startingTimeHandle2 == -1) return;

        setConstrains(criticalStateHandle2, startingTimeHandle2+2, resInitialHeuristics, handle2, consOther, handle1);

        setConstrains(nextSolutionState   , startingTimeHandle2+2, resInitialHeuristics, handle2, consOther, handle1);
    ...
    if (resPrevHandle1.second == -1) return; // Should never trigger
