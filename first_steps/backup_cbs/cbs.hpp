...
class CBS {
 public:
  ...
  bool search(const std::vector<State>& initialStates, std::vector<PlanResult<State, Action, int>>& solution, std::vector<Constraints>& constraints, bool replan=false) {
    ...
    for (const auto& asdf : start.constraints) {
      
      for (const auto& a : asdf.vertexConstraints) {
       std::cout << a << std::endl;
      }
    }
    ...
    while (!open.empty()) {
      ...
      // Conflict conflict;
      if (!m_env.getFirstConflict(P.solution, resultConstraints, resultDoubleConstraints)) {
      ...
      for (const auto& c : resultDoubleConstraints) {
        ...
        if (searchLowLevel(newNode, handle1, initialStates) && searchLowLevel(newNode, handle2, initialStates)) {
      ...
      for (const auto& c : resultConstraints) {
        ...
        if (searchLowLevel(newNode, handle, initialStates)) {
          auto h = open.push(newNode);
          (*h).handle = h;
        }
  ...
  bool searchLowLevel(auto& newNode, size_t handle, const auto& initialStates) {
