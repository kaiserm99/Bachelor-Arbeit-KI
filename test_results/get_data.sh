...
for x in {1..14}
do
    # tail -1 flatland_test$1/result-$x | awk {'print $3'}
    # tail -1 flatland_test$1/result-$x | awk {'print $6'}
    var=$(tail -1 flatland_test$1/result-$x)
    ...
    time=$(tail -n3 flatland_test$1/result-$x | head -n1 | awk {'print $5'})
    time=$(sed 's/.\{3\}$//' <<<"$time")
