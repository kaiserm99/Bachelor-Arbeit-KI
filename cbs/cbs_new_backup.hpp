 ...
 public:
  ...
  bool search(const std::vector<State>& initialStates,
    ...
    while (!open.empty()) {
      ...
      std::multimap<size_t, Constraints> resultConstraints;
      ...
      std::cout << std::endl << "Final Constraints:" << std::endl;
      for (const auto& cons : resultConstraints) {
        std::cout << "Agent " << cons.first << ": " << cons.second<< std::endl;
        // for (const auto& state : P.solution[cons.first].states) {
