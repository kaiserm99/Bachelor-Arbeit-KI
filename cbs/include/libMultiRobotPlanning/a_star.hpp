...
class AStar {
 public:
  ...
  bool search(const State& startState,
              PlanResult<State, Action, Cost>& solution, int speed, int replannigSize, int replanNext, Cost initialCost = 0) {
    ...
    while (!openSet.empty()) {
      ...
      if (m_env.isSolution(current.state)) {
        ...
        std::cout << "A-Star Replanning size: " << replannigSize << std::endl;

        while (iter != cameFrom.end()) {
          // std::cout << "(" << iter->first.y << ", " << iter->first.x << "), a: " << std::get<1>(iter->second) << std::endl;
        ...
        for (int i = 0; i < replannigSize; i++) {

          State currentState = solution.states[i].first;
          Action currentAction = solution.actions[i].first;

          states.push_back(std::make_pair<>(State(i, currentState.y, currentState.x , currentState.dir), 1));
          actions.push_back(std::make_pair<>(currentAction, 1));

        }

        std::cout << replanNext << std::endl;
        for (size_t i = replannigSize + replanNext; i < solution.states.size() - 1; i++) {
          ...
          for (int s = 0; s < speed; s ++) {
            ...
            states.push_back(std::make_pair<>(State((i - replannigSize - replanNext) * speed + s + replannigSize, currentState.y, currentState.x , currentState.dir), 1));
            actions.push_back(std::make_pair<>(currentAction, 1));

          }
        }

        // Make sure to add the last State only once
        int lastStateIndex = states.back().first.time + 1;

        std::cout << "Last State Index: " << lastStateIndex << std::endl;

        State currentState = solution.states.back().first;
        states.push_back(std::make_pair<>(State(lastStateIndex, currentState.y, currentState.x , currentState.dir), 1));


        // Copy the states and the actions
        solution.states = states;
        solution.actions = actions;


        solution.cost = solution.states.size() - 1;
        solution.fmin = current.fScore;

        std::cout << std::endl << "New Solution:" << std::endl;
        for (const auto& s : solution.states) std::cout << s.first << std::endl;
