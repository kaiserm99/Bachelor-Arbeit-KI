...
for x in {1..14}
do
    tst=$(tail -1 flatland_test$1-MAPF/result-$x | head -n1 | awk {'print $1'})
    ...
    rew=$(tail -3 flatland_test$1-MAPF/result-$x | head -n1 | awk {'print $2'})
    steps=$(tail -2 flatland_test$1-MAPF/result-$x | head -n1 | awk {'print $2'})
    time=$(tail -1 flatland_test$1-MAPF/result-$x | head -n1 | awk {'print $2'})
...
#$(counter=1; for x in 2 4 6 8 10 11 12 14 15 16 17 18 19 20; do agt_count=$(sed "${counter}q;d" $filename | awk {'print $1'}); echo "set label '$(sed "${counter}q;d" $filename | awk {'print $3'})' at $x,$(bc -l <<< $(sed "${counter}q;d" $filename | awk {'print $2'})"+0.3");"; counter=$((counter+1)); done)